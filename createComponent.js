...
if (args.length > 0 && args[0].toLowerCase() === "help") {
  console.log(`
Usage examples:
  create-component MyComponent                          # Create only page.tsx
  create-component MyComponent hk2 fm3 pg2              # Create only hooks, forms, and pages folders with specified number of files
  create-component MyComponent --template-fsd           # Create only page.tsx with Feature-Sliced Design template
  create-component MyComponent hk2 pg2 --template-fsd   # Create hooks and pages folders with Feature-Sliced Design templates
  create-component MyComponent rp3                      # Create page.tsx plus 2 additional files in the root folder

Folder prefixes:
  hk = hooks       - Custom React hooks
  fm = forms       - Form-related hooks and components
  pg = pages       - Page components
  ui = ui          - UI components
...
if (!componentName) {
  console.log(
    "❌ Please provide a component name!\nExample: node ~/scripts/createFilesFolders.js MyComponent [--template-fsd | --template-bullet]"
  );
  process.exit(1);
}
...
function createDefaultTemplateFile(templateType, filePath, templateSubdir) {
  ...
  if (templateSubdir === "fsd") {
    ...
    switch (templateType) {
      ...
      case "pages":
        templateContent = `// Bullet Template for pages`;
        break;
...
function generateRootContent(componentName, index) {
  const component = {
    name: componentName,
    kebabName: toKebabCase(componentName),
  };

  return loadTemplate("root", component, index);
}
...
function parseFileCountArgs(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (folderPrefixMap[prefix] && !isNaN(count)) {
...
function getFileName(folder, kebabName, index) {
  ...
  switch (folder) {
    ...
    case "pages":
      return `${kebabName}-page${indexSuffix}.tsx`;
    case "mutations":
...
function createFolderStructure(basePath, folders, fileCountMap) {
  ...
  const pageFilePath = path.join(basePath, "page.tsx");
  ...
  for (let i = 1; i < rootFileCount; i++) {
    ...
    if (!fs.existsSync(additionalFilePath)) {
      const content = generateRootContent(componentName, i);
